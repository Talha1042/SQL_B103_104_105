...
from sirketler inner join siparisler   -- from ve inner join kısmini degistirebiliriz.
...
-- siparis_tarihleri ile yeni bir tablo olusturun
SELECT sirketler.sirket_isim , siparisler.siparis_id , siparisler.siparis_tarihi
from sirketler left join siparisler   -- from ve inner join kısmini degistirebiliriz.
on sirketler.sirket_id = siparisler.sirket_id;
...
-- genellikle left join kullaniriz.
-- FULL JOIN
select sirketler.sirket_isim, siparisler.sirket_id, siparisler.siparis_tarihi
...
select * from musteriler
where isim like '_atma'   -- _ yerine % isareti de koyabilirdik.
...
-- QUERY yazin
select * from kelimeler
where kelime ~* 'h[a-k]t'  -- * buyuk kucuk harf farettrimiyo
